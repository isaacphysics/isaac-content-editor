...
interface CollapsableArg {
  preview: { open: boolean; toggle: () => void };
}
function useCollapsableDragElement(appContext: CollapsableArg): [(col: number) => HTMLElement, number | undefined] {
  ...
  useEffect(
    function closePreviewIfPanelCollapsed() {
      if (collapsed === 2 && appContext.preview.open) {
        appContext.preview.toggle();
      }
    },
...
export function EditorScreen() {
  ...
  const setCurrentDoc = useCallback(
    (content: Content | string, invertible = false) => {
      if (invertible) {
    ...
    <SWRConfig value={{ fetcher, revalidateOnFocus: false, revalidateOnReconnect: false }}>
      <AppContext.Provider value={appContext}>
        ...
        >
          <LeftMenu />
          <ErrorBoundary
            FallbackComponent={buildPageError(selection?.path)}
            onResetKeysChange={() => setCurrentContent({})}
            onReset={() => setCurrentContent({})}
            resetKeys={[selection]}
          >
            <div>
              {hasFileOpen ? (
                ...
                <div className={`${styles.centered} mt-5`}>Choose a file on the left to edit</div>
              )}
            </div>
          </ErrorBoundary>
          <div className={`${styles.flexFill} h-100`}>{showPreview && previewComponent}</div>
        </Split>
